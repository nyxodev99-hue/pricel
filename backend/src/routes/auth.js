...
authRoute.post('/register', async (c) => {
  ...
  if (!allowed) {
    return c.json({ error: 'Un compte existe déjà.' }, 429);
  }

  const deviceToken = c.req.header('X-Device-Token') || null;
  if (!(await isDeviceTokenAvailable(db, deviceToken))) {
    return c.json({ error: 'Un compte existe déjà.' }, 429);
  }
  ...
  await recordDeviceSignup(db, deviceToken, user.id);
